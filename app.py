...
# Cấu hình trang
st.set_page_config(
    page_title="Hệ thống tóm tắt văn bản tiếng Việt", page_icon="📰", layout="wide"
)
...
# Header
st.markdown(
    '<h1 class="main-header">📰 Hệ thống Tóm tắt Văn bản Tiếng Việt</h1>',
    unsafe_allow_html=True,
)
st.markdown("---")
...
with st.sidebar:
    ...
    api_key = st.text_input(
        "Gemini API Key", type="password", help="Nhập API key của Google Gemini"
    )
    ...
    if "Extractive" in method:
        ...
        summary_length = st.select_slider(
            "Độ dài tóm tắt", options=["Ngắn", "Trung bình", "Dài"], value="Trung bình"
        )
...
with col2:
    ...
    if st.button("🚀 Tóm tắt văn bản", type="primary", use_container_width=True):
        if not input_text.strip():
            ...
            st.error(
                "⚠️ Vui lòng nhập Gemini API Key để sử dụng phương pháp Abstractive!"
            )
        else:
            with st.spinner("⏳ Đang xử lý..."):
                ...
                reduction = round(
                    (1 - summary_word_count / len(input_text.split())) * 100, 1
                )

                st.caption(
                    f"📊 Tóm tắt: {summary_word_count} từ, {summary_sent_count} câu | Giảm {reduction}%"
                )
